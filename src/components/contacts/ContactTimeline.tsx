...
export function ContactTimeline({ contactId }: ContactTimelineProps) {
  const { entries, isLoading, createEntry } = useTimelineEntries({ contactId });
    ...
    <div className="space-y-6">
      ...
      {filteredEntries.length === 0 ? (
        <EmptyState
          ...
          description={
            filter === 'all'
              ? "No hay interacciones registradas con este contacto aún."
      ...
      <Dialog open={isAddNoteOpen} onOpenChange={setIsAddNoteOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Añade una nota al historial de este contacto
            </DialogDescription>
          </DialogHeader>
          <Textarea
