...
const BOOT_TIMEOUT_MS = 10000; // 10 seconds
...
export function AppLayout({ children }: AppLayoutProps) {
  ...
  // Only block on auth and initial team loading
  const isLoading = authLoading || teamLoading;

  // Boot timeout - if loading takes too long, show recovery UI
  useEffect(() => {
    if (!isLoading) {
  ...
  // Error or timeout state - show recovery UI
